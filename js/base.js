	...
	Confirm.prototype = {
		initialize: function(parent, options) {
			...
			this.useragent.innerHTML = 'Usted esta usando el navegador' + Browsers;
		},
		...
		reportUseragent: function() {
			...
			new Feedback(this.confirm, {
				suggestion:	'Estoy usando' + ' ' + Browsers,
				...
				if (typeof tests[i] != 'string') {
					...
					if (typeof tests[i].items != 'undefined') {
						...
						if (results[0] == results[1])
							cell.innerHTML = '<div class="result">Si <span class="check">✔</span></div>';
						else if (results[1] == 0)
							cell.innerHTML = '<div class="result">No <span class="ballot">✘</span></div>';
						else
							cell.innerHTML = '<div class="result"><span class="partially">Parcial</span> <span class="partial">○</span></div>';
					}
						...
						if (match = (new RegExp(key + '=(-?[0-9]+)')).exec(data.results)) {
							...
							if (result & YES) {
								switch(true) {
									case !! (result & BUGGY):		cell.innerHTML = '<div class="result">Buggy <span class="buggy"></span></div>'; break;
									case !! (result & OLD):			cell.innerHTML = '<div class="result">Parcial <span class="partial">○</span></div>'; count[1]++; break;
									case !! (result & PREFIX):		cell.innerHTML = '<div class="result">Prefijado <span class="check">✔</span></div>'; count[1]++; break;
									default:						cell.innerHTML = '<div class="result">Si <span class="check">✔</span></div>'; count[1]++; break;
								}
							}
							else {
								switch(true) {
									case !! (result & UNKNOWN):		cell.innerHTML = '<div class="result">Desconocido <span class="partial">?</span></div>'; break;
									case !! (result & BLOCKED):		cell.innerHTML = '<div class="result">No functional <span class="buggy">!</span></div>'; break;
									case !! (result & DISABLED):	cell.innerHTML = '<div class="result">Desactivado <span class="ballot">✘</span></div>'; break;
									default:						cell.innerHTML = '<div class="result">No <span class="ballot">✘</span></div>'; break;
								}
							}
						} else {
							cell.innerHTML = '<div><span class="partially">Desconocido</span> <span class="partial">?</span></div>';
						}
					}
